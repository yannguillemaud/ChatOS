...
#### Server Message
If the private connection request has been accepted, the server wil send to both clients a message containing the port number they have to be connected (the ports numbers will be different for the two clients).

It will have the following format :
- OPCode : 11 (byte)
- Number of bytes in the other client pseudonym endoded (int)
